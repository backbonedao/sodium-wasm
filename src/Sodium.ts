...
function emscripten_asm_const_int(
	code: number,
	sigPtr: number,
	argbuf: number,
): number {
...
export function crypto_auth_verify(
	...
	const inpptr = mallocAndCopy(inp);
	const kptr = mallocAndCopy(k);
	const ret = Sodium.crypto_auth_verify(
		hptr,
		inpptr,
		BigInt(inp.byteLength),
		kptr,
	);
	free(hptr);
	free(inpptr);
...
export function crypto_box_easy(
	...
	const ret = Sodium.crypto_box_easy(
		cptr,
		mptr,
		BigInt(m.byteLength),
		nptr,
...
export function crypto_box_open_easy(
	...
	const pkptr = mallocAndCopy(pk);
	const skptr = mallocAndCopy(sk);
	const ret = Sodium.crypto_box_open_easy(
		mptr,
		cptr,
		BigInt(c.byteLength),
		nptr,
...
export function crypto_box_detached(
	...
	const pkptr = mallocAndCopy(pk);
	const skptr = mallocAndCopy(sk);
	const ret = Sodium.crypto_box_detached(
		cptr,
		macptr,
		mptr,
		BigInt(m.byteLength),
...
export function crypto_box_open_detached(
	...
	const pkptr = mallocAndCopy(pk);
	const skptr = mallocAndCopy(sk);
	const ret = Sodium.crypto_box_open_detached(
		mptr,
		cptr,
		macptr,
		BigInt(c.byteLength),
...
export function crypto_generichash(
	...
	const inpptr = mallocAndCopy(inp);
	const keyptr =
		key === null || typeof key === "undefined" ? null : mallocAndCopy(key);
	const keylen =
		key === null || typeof key === "undefined" ? 0 : key.byteLength;
	const ret = Sodium.crypto_generichash(
		outptr,
		out.byteLength,
		inpptr,
		BigInt(inp.byteLength),
...
export const crypto_hash_sha256_STATEBYTES =
...
export const crypto_hash_sha512_STATEBYTES =
...
export const crypto_onetimeauth_STATEBYTES =
	Sodium.crypto_onetimeauth_statebytes();
...
export function crypto_onetimeauth_verify(
	...
	const inpptr = mallocAndCopy(inp);
	const kptr = mallocAndCopy(k);
	const ret = Sodium.crypto_onetimeauth_verify(
		hptr,
		inpptr,
		BigInt(inp.byteLength),
		kptr,
...
export const crypto_pwhash_ALG_ARGON2I13 = 1;
...
export function crypto_pwhash_str_verify(
	...
	const passwdptr = mallocAndCopy(passwd);
	const ret = Sodium.crypto_pwhash_str_verify(
		strptr,
		passwdptr,
		BigInt(passwd.byteLength),
	);
...
export function crypto_pwhash_str_needs_rehash(
	...
	const strptr = mallocAndCopy(str);
	const ret = Sodium.crypto_pwhash_str_needs_rehash(
		strptr,
		BigInt(opslimit),
		memlimit,
	);
...
export function crypto_pwhash_scryptsalsa208sha256_str_verify(
	...
	const strptr = mallocAndCopy(str);
	const passwdptr = mallocAndCopy(passwd);
	const ret = Sodium.crypto_pwhash_scryptsalsa208sha256_str_verify(
		strptr,
		passwdptr,
		BigInt(passwd.byteLength),
	);
...
export function crypto_pwhash_scryptsalsa208sha256_str_needs_rehash(
	...
	const strptr = mallocAndCopy(str);
	const ret = Sodium.crypto_pwhash_scryptsalsa208sha256_str_needs_rehash(
		strptr,
		BigInt(opslimit),
		memlimit,
	);
...
export function crypto_secretbox_open_easy(
	...
	const nptr = mallocAndCopy(n);
	const kptr = mallocAndCopy(k);
	const ret = Sodium.crypto_secretbox_open_easy(
		mptr,
		cptr,
		BigInt(c.byteLength),
		nptr,
...
export function crypto_secretbox_open_detached(
	...
	const nptr = mallocAndCopy(n);
	const kptr = mallocAndCopy(k);
	const ret = Sodium.crypto_secretbox_open_detached(
		mptr,
		cptr,
		macptr,
		BigInt(c.byteLength),
...
export const crypto_secretstream_xchacha20poly1305_TAG_MESSAGE = Buffer.from([
	Sodium.crypto_secretstream_xchacha20poly1305_tag_message(),
]);
export const crypto_secretstream_xchacha20poly1305_TAG_PUSH = Buffer.from([
	Sodium.crypto_secretstream_xchacha20poly1305_tag_push(),
]);
export const crypto_secretstream_xchacha20poly1305_TAG_REKEY = Buffer.from([
	Sodium.crypto_secretstream_xchacha20poly1305_tag_rekey(),
]);
export const crypto_secretstream_xchacha20poly1305_TAG_FINAL = Buffer.from([
...
export const crypto_sign_STATEBYTES = Sodium.crypto_sign_statebytes();
...
export function crypto_sign_open(
	...
	const pkptr = mallocAndCopy(pk);
	const ret = Sodium.crypto_sign_open(
		mptr,
		null,
		smptr,
		BigInt(sm.byteLength),
		pkptr,
	);
	copyAndFree(mptr, m);
	free(smptr);
...
export function crypto_sign_verify_detached(
	...
	const mptr = mallocAndCopy(m);
	const pkptr = mallocAndCopy(pk);
	const ret = Sodium.crypto_sign_verify_detached(
		sigptr,
		mptr,
		BigInt(m.byteLength),
		pkptr,
...
export function crypto_stream_chacha20_ietf(
	c: Uint8Array,
	n: Uint8Array,
	k: Uint8Array,
): void {
...
export function crypto_stream_xchacha20(
	c: Uint8Array,
	n: Uint8Array,
	k: Uint8Array,
): void {
...
export function crypto_stream_salsa20(
	c: Uint8Array,
	n: Uint8Array,
	k: Uint8Array,
): void {
...
export function crypto_stream_salsa20_xor(
	c: Uint8Array,
	m: Uint8Array,
	n: Uint8Array,
	k: Uint8Array,
...
export function crypto_box_seal_open(
	...
	const pkptr = mallocAndCopy(pk);
	const skptr = mallocAndCopy(sk);
	const ret = Sodium.crypto_box_seal_open(
		mptr,
		cptr,
		BigInt(c.byteLength),
		pkptr,
